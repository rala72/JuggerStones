...
public class PreferenceFragment extends XpPreferenceFragment
    ...
    private void initListener() {
        ...
        final Preference pref_email =
            findPreference(JuggerStonesApp.PREFS.EMAIL.toString());
        pref_email.setOnPreferenceClickListener(preference -> {
            JuggerStonesApp.sendEmail(requireContext());
            return true;
        });
        final Preference pref_version =
            findPreference(JuggerStonesApp.PREFS.VERSION.toString());
        pref_version.setTitle(getString(R.string.pref_version,
            JuggerStonesApp.getVersion(requireContext())
        ));
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        getListView().addItemDecoration(
            new PreferenceDividerDecoration(requireContext()).drawBetweenItems(false)
        );
    }
    ...
    private void updateSoundStoneCountdown() {
        final String value = ((SoundPreferenceList)
    ...
    private void validatePreferences(String key) {
        if (key.equals(JuggerStonesApp.PREFS.MODE_CUSTOM.toString())) {
            ...
            long value = mode_custom.getText().trim().isEmpty() ?
                min : Long.parseLong(mode_custom.getText().trim());
        ...
        if (key.equals(JuggerStonesApp.PREFS.INTERVAL_CUSTOM.toString())) {
            ...
            double value = interval_custom.getText().trim().isEmpty() ?
                min : Double.parseDouble(interval_custom.getText().trim());
    ...
    private void updatePreferencesEnabled(String key) {
        if (key == null || key.equals(JuggerStonesApp.PREFS.MODE.toString())) {
            ...
            findPreference(JuggerStonesApp.PREFS.REVERSE.toString())
                .setEnabled(!"-1".equals(mode.getValue()));
        }
    ...
    private void updateLanguageText() {
        ...
        languageMap.put(
            getString(R.string.language_english), getString(R.string.language_english_en)
        );
        languageMap.put(
            getString(R.string.language_german), getString(R.string.language_german_en)
        );
        languageMap.put(
            getString(R.string.language_spanish), getString(R.string.language_spanish_en)
        );
        languageMap.put(
            getString(R.string.language_french), getString(R.string.language_french_en)
        );
        languageMap.put(
            getString(R.string.language_portuguese), getString(R.string.language_portuguese_en)
        );
        // endregion
        final String[] languages = getResources().getStringArray(R.array.array_languages_array);
        for (int i = 0; i < languages.length; i++)
            languages[i] = String.format(format,
                languages[i], languageMap.get(languages[i])
            );
